...
def predict_all_bm25(
    dataset_path, bm25_index_path, eval_segment="test", k1=None, b=None, topk=None
):
...
def predict_monot5(reranker, doc, candidate_dir, config):
    ...
    for c in result:
        ...
        monot5_score[cand_id] = max(monot5_score[cand_id], np.exp(c.score) * 100)
    return monot5_score
...
def predict_bert(reranker, doc, candidate_dir, config):
    if config["train_uncased"]:
        doc = doc.lower()
    query = Query(doc)
    texts = []
    ...
    bert_score = defaultdict(lambda: 0)
    result = reranker.rerank(query, texts)
    for c in result:
        cand_id = c.metadata["docid"]
        bert_score[cand_id] = max(bert_score[cand_id], np.exp(c.score) * 100)
    return bert_score


def predict_all_bert(dataset_path, ckpt_path, eval_segment="test"):
    model = BertForSequenceClassification.from_pretrained(ckpt_path).to(device).eval()
    ...
    for case in cases_dir:
        ...
        candidate_dir = corpus_dir / case / "paragraphs"
        score = predict_bert(reranker, base_case_data, candidate_dir, config)
        bert_scores[case] = score
    return bert_scores
...
def eval_bm25(dataset_path, bm25_index_path, eval_segment="test"):
    ...
    for case in cases_dir:
        ...
        for i, k in enumerate(top_k):
            ...
            tp[i] += np.sum(
                [1 if a == b and a == 1 else 0 for a, b in zip(pred, label)]
            )
            fp[i] += np.sum(
                [1 if a != b and a == 1 else 0 for a, b in zip(pred, label)]
            )
            fn[i] += np.sum(
                [1 if a != b and a == 0 else 0 for a, b in zip(pred, label)]
            )
...
def eval_bm25_end_model_ranking(
    dataset_path,
    bm25_scores,
    scores,
    top_k=1,
...
def eval_bm25_end_model(
    ...
    if top_k is None:
        ...
        if eval_segment in ["val", "test"]:
            bm25_test_index_path = "../data/bm25_indexes/coliee_task2/2024/val"
            bm25_test_scores = predict_all_bm25(
                dataset_path, bm25_test_index_path, eval_segment="val"
            )
            test_scores = predict_func(dataset_path, ckpt_path, eval_segment="val")
        ...
        return eval_bm25_end_model_ranking(
            dataset_path,
            bm25_scores,
            scores,
            top_k,
...
if __name__ == "__main__":
    ...
    elif args.model in ["monot5", "bert"]:
        eval_bm25_end_model(
            args.dataset_path,

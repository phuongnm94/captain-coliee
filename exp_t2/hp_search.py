...
def run_trial(trial):
    ...
    cmd = f"{sys.executable} train.py --config_path {tmp_dir}/train_configs.json -s {tmp_dir}"
    try:
    ...
    val_res, test_res, ckpt, conf = evaluate_ckpts(
        os.path.join(tmp_dir, "ckpt"),
        os.path.join(root_dir, configs["dataset_path"]),
        os.path.join(root_dir, configs["bm25_index_dir"]),
        segment=val_segment,
...
study = optuna.create_study(study_name="flan-monot5_ft", direction="maximize")
study.optimize(
    run_trial, n_trials=n_trial, gc_after_trial=True, callbacks=[save_best_model]
)
...
print(
    f"Mean val: {np.mean(agg_val_res, axis=0)} - Std val: {np.std(agg_val_res, axis=0)}"
)
print(
    f"Mean test: {np.mean(agg_test_res, axis=0)} - Std val: {np.std(agg_test_res, axis=0)}"
)

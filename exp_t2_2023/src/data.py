...
def get_task2_data(dataset_path, segment="test"):
    if segment == "train":
        start_idx, end_idx = 0, 525
    elif segment == "val":
        start_idx, end_idx = 525, 625
    elif segment == "test":
...
def build_task2_training_dataset(dataset_path, training_samples_file,
    ...
    for case in cases_dir:
        ...
        case_dict = {"id": case, "text": base_case_data, "pos_candidates": [], "neg_candidates": []}
        ...
        for cand_case in candidate_cases:
            ...
            cand_case_data = preprocess_case_data(cand_case_file, uncased=train_uncased,
...
class TrainDataset(Dataset):
    def __init__(self, root_dir, dataset_path, training_samples_file, num_pairs_per_batch, ns_strategy,
        ...
        self.training_data = []
        self.ps = {sample["id"]: [pos["id"] for pos in sample["pos_candidates"]]
                   for sample in self.data}
        self.ps_iter = copy.deepcopy(self.ps)
        self.ns = {sample["id"]: [neg["id"] for neg in sample["neg_candidates"]]
                   for sample in self.data}
    ...
    def create_training_dataset(self, model, epoch):
        ...
        if self.ns_strategy == "random":
            ...
            self.seq_negative_sampling_dataset()
        ...
        for sample in self.data:
            ...
            if len(self.ps[sample["id"]]) == 1:
                ...
                if len(self.ps_iter[sample["id"]]) > 1:
                    ...
                    self.ps_iter[sample["id"]] = copy.deepcopy(self.ps[sample["id"]][diff:])
                pos_cands = [pos["text"] for pos in sample["pos_candidates"] if pos["id"] in pos_cand_id]
            for t in pos_cands:
            ...
            neg_cands = [neg["text"] for neg in sample["neg_candidates"] if neg["id"] in neg_cands_id]
            for t in neg_cands:
...
class MonoT5BatchCollator:
    ...
    def __call__(self, batch, return_tensors=None):
        texts = [self.pattern.format(example[0], example[1]) for b in batch for example in b]
        ...
        tokenized["inst_w"] = torch.tensor(flatten([(1, example[3]) for b in batch for example in b]))
...
class BertBatchCollator:
    ...
    def __call__(self, batch, return_tensors=None):
        ...
        bert_encoded = [
            (self.bert_tokenizer.encode(t[0], add_special_tokens=False,
                max_length=512, truncation=True),
             self.bert_tokenizer.encode(t[1], add_special_tokens=False,
                max_length=512, truncation=True)) for t in texts]
        print(bert_encoded[0])
        ...
        tokenized = self.tokenizer.batch_encode_plus(
            bert_encoded, padding=True, truncation='longest_first',
            return_tensors='pt', max_length=self.max_length)

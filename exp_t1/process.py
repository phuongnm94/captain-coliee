...
for split in splits:
    names = os.listdir(f"dataset/c2023/{split}_files")
    total = 0
    for name in names:
        with open(f"dataset/c2023/{split}_files/{name}", "r", encoding="utf-8") as f:
            t = f.read()
            idx_ = t.find("[1]")
            if idx_ != -1:
                t = t[idx_:]
        ...
        with open(f"dataset/processed/{name}", "w+", encoding="utf-8") as f:
            f.write(txt)

        total += 1
        if total % 100 == 0:
            print(f"split {split}, {total}, and total {len(names)}")